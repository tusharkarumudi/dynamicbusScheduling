...
class LookAheadHandler(object):
	...
	def update_timetables_of_bus_line(self, bus_line=None, bus_line_id=None):
		...
		:return: None
		"""
		print("I am inside the Update_timetables_of_bus_line")
		if bus_line is None and bus_line_id is None:
		    print("when both are none")
		    return None
		elif bus_line is None:
		    print("when bus_line is none")
		    bus_line = self.mongodb_database_connection.find_bus_line_document(bus_line_id=bus_line_id)
		else:
		    print("when bus_line_id is none")
		    bus_line_id = bus_line.get('u\'line_id')
		...
		timetables = self.mongodb_database_connection.find_timetable_documents(bus_line_ids=[bus_line_id])
			...
			#print("timetable is not none")
			#print(timetables)
		#else:
			#print("timetables is none")


		travel_requests = self.get_travel_requests_of_timetables(timetables=timetables)
	...
	#akshay changed	#log(module_name='look_ahead_handler', log_type='DEBUG',
		#    log_message='update_timetable_documents (mongodb_database): ok')			
			
			
	def generate_new_timetables_based_on_travel_requests(self,current_timetables, travel_requests):
	...
	def get_travel_requests_of_timetables(self,timetables):
	    """
		...
		if(travel_requests_of_timetable is not None):
